...
-- 글삭제시 답변글까지 삭제하기 (삭제될 글의 GROUP,STEP,INDENT 필요)
DELETE FROM BOARD WHERE BOARDGROUP=1 AND (BOARDSTEP>=1 AND 
  BOARDSTEP<(SELECT NVL(MIN(BOARDSTEP),99) FROM BOARD WHERE BOARDGROUP=1 AND BOARDSTEP>1 AND BOARDINDENT<=1));
-- 삭제되지 않고 남은 글 STEP조정 (위의 로직에서 삭제된 갯수 필요)
UPDATE BOARD SET BOARDSTEP = BOARDSTEP-1 WHERE BOARDGROUP=1 AND BOARDSTEP>1;
