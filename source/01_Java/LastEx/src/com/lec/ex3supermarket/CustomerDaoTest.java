...
public class CustomerDaoTest {
	public static void main(String[] args) {
		...
		String ctel = "010-1414-1414", cname = "이순신";
		result = dao.insertCustomer(ctel, cname);
		System.out.println(result == CustomerDao.SUCCESS? cname+"님 회원가입 감사. 포인트 1000점 넣어드림":
														  cname+"님 회원가입 실패");
		...
		if(result==CustomerDao.SUCCESS) { // 물품구매 성공시 성공메세지와 함께 고객정보 출력
			CustomerDto customer = dao.getCustomer(1);
			System.out.println("물품구매 감사합니다 고객님정보는 아래와 같이 업데이트 되었습니다");
			System.out.println("아이디\t전화\t\t이름\t포인트\t누적구매액\t고객레벨\t레벨업을위한남은구매액");
			System.out.println(customer);
		} else {
		...
		System.out.println("고객레벨명 : "+dao.getGrades());
