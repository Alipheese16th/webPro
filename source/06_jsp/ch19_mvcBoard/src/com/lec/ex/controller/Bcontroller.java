...
public class Bcontroller extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(command.equals("/modify.do")) { // 글수정 DB에 접근
			service = new BModifyService();
			service.execute(request, response);
			//viewPage = "list.do";  // requestScope.modifyResult, param.pageNum, param....
			viewPage = "contentView.do";
		}else if(command.equals("/delete.do")) {
			service = new BDeleteService(); // DB에 글 삭제
			service.execute(request, response);
			viewPage = "list.do";
		}else if(command.equals("/replyView.do")) { // 답변글쓰기 위한 view
			service = new BReplyViewService();
			service.execute(request, response);
			viewPage = "board/reply_view.jsp"; // requestScope.replyBoard, param.pageNum, param.bid
