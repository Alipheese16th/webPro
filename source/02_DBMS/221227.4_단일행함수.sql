...
SELECT * FROM DUMMY;
DESC DUMMY;
SELECT ABS(-1) FROM DUMMY; -- 절대값 함수
SELECT FLOOR(34.5678) FROM DUAL; -- 소수점에서 내림(버림)
SELECT FLOOR(34.5678*100)/100 FROM DUAL; --소수점 두번째에서 자리에서 내림(버림)
...
SELECT ROUND(34.5678,2) FROM DUAL; -- 소수점 두번째 자리까지 나오는 반올림
SELECT ROUND(34.5678,-1) FROM DUAL; -- 일의자리 반올림

SELECT MOD(9,2) FROM DUAL; -- 나머지 연산자
    -- ex. 홀수년도에 입사한 사원의 모든 정보 출력
SELECT * FROM EMP WHERE MOD(TO_CHAR(HIREDATE,'RR'),2) <> 0;

-- 2. 문자관련함수
-- ① 대소문자 관련
SELECT INITCAP('WELCOME TO ORACLE') FROM DUAL; -- 첫문자만 대문자
...
SELECT * FROM EMP WHERE UPPER(ENAME) = 'SCOTT'; -- 혹시라도 데이터에 대소문자가 확인이 되지않으면 비교할수있는 방법 
...
SELECT SUBSTR(123, 2, 1) FROM DUAL;  -- 숫자도 가능(숫자를 문자로 바꿔 2번째 부터 1글자 추출) 자동 형변환
...
SELECT * FROM EMP WHERE INSTR(HIREDATE, '09', 7) = 7;
    -- 9일이 아닌 다른날에 입사한 사원의 모든필드(INSTR이용)
...
SELECT RPAD(ENAME,11,'-'), LPAD(SAL,6,'*') FROM EMP;
    ...
    -- ex. 사번, 이름(7788 S****)
SELECT EMPNO, RPAD(SUBSTR(ENAME,1,1),LENGTH(ENAME),'*') NAME, ENAME FROM EMP;
    -- ex. 사번, 이름, 직책(9자리확보,뒷1자리만보임), 입사일 -- LPAD, SUBSTR, LENGTH, CONCAT
SELECT EMPNO, ENAME, LPAD(LPAD(SUBSTR(JOB,-1,1),LENGTH(JOB),'*'),9) JOB, 
    RPAD(SUBSTR(HIREDATE,1,6),LENGTH(HIREDATE),'*') DAY FROM EMP;
SELECT EMPNO, ENAME, LPAD(LPAD(SUBSTR(JOB,-1,1),LENGTH(JOB),'*'),9) JOB, 
    SUBSTR(HIREDATE,1,6) || '**' DAY FROM EMP;
SELECT EMPNO, ENAME, LPAD(LPAD(SUBSTR(JOB,-1,1),LENGTH(JOB),'*'),9) JOB, 
    TO_CHAR(HIREDATE,'RR/MM/') || '**' DAY FROM EMP;
    -- ex. 이름의 세번째 자리 글자가 R인 사원의 모든 필드 출력(LIKE 이용, INSTR이용, SUBSTR이용)
...
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) MONTH FROM EMP;
    -- ex. 이름, 입사한 이후 회서에서 받은 돈(SAL은 1년에 12번, COMM은 2번)을 출력
SELECT ENAME, SAL*TRUNC(MONTHS_BETWEEN(SYSDATE,HIREDATE)) + NVL(COMM,0)*TRUNC((MONTHS_BETWEEN(SYSDATE,HIREDATE))/6) PAY FROM EMP;
...
SELECT ROUND(SYSDATE, 'MONTH') FROM DUAL;  -- 결과 : 가까운 1일 (1~15일 : 이번달 1일/ 16~31일 : 다음달 1일)
SELECT ROUND(SYSDATE, 'DAY') FROM DUAL;  -- 결과 : 가까운 일요일 (월화수 / 목금토)
SELECT ROUND(SYSDATE) FROM DUAL;  -- 결과 : 가까운 0시0분
SELECT TRUNC(SYSDATE, 'YEAR') FROM DUAL;  -- 올해 1월1일
...
SELECT TO_CHAR(TRUNC(SYSDATE),'RR/MM/DD HH24:MI') FROM DUAL;  -- 오늘 0시0분
    ...
    -- ex1. 이름, 입사일, 첫월급날(11일) : 10일 이전이면 이번달 11일 / 11일 이후면 다음달 11일
...
SELECT TO_CHAR(1200, 'L9,999') FROM DUAL;  -- 지역별 통화
DESC EMP;  -- SAL NUMBER(7,2) ; 전체 자리수는 7자리 / 앞5자리 소수점2자리
...
SELECT '3,456'+1 FROM DUAL;  -- 에러 : , 가 있어서 자동 형변환이 안되는 문자
SELECT TO_NUMBER('3,456','9,999')+1 FROM DUAL;  -- 문자를 숫자로 변환
...
SELECT W.ENAME, NVL(M.ENAME,'CEO') FROM EMP W, EMP M WHERE W.MGR = M.EMPNO(+);
    -- ex. 사원이름, 직속상사 사번(직속상사가 없으면 'CEO'로 출력)
...
SELECT SYSDATE, TO_CHAR(SYSDATE,'DD') FROM DUAL;  -- 일 2자리를 문자로 추출
    -- (2) 레벨 별 출력
SELECT LEVEL, LPAD('ㄴ',LEVEL*2) || ENAME FROM EMP
...
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) MONTH, 
    SAL, SAL* TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) SALTOTAL FROM EMP;
...
SELECT ENAME, JOB, HIREDATE, 
     CONCAT(TO_CHAR(HIREDATE,'DY'),'요일') "입사한 요일은?" FROM EMP;
...
SELECT ENAME, JOB, SAL, NVL(COMM,0) BONUS, SAL+NVL(COMM,0) TOTAL FROM EMP;

-- 9.사원 테이블의 사원명에서 2번째 문자부터 3개의 문자를 추출하시오. 
SELECT SUBSTR(ENAME,2,3) "3글자" FROM EMP;
...
--7521	    WARD	    ******1250
--……. 
SELECT EMPNO, ENAME, LPAD(SAL,10,'*') SAL FROM EMP;

-- 12. 다음과 같은 결과를 검색할 수 있는 SQL 문장을 작성하시오
-- EMPNO	 ENAME 	입사일
-- 7369	  SMITH		1980-12-17
-- ….
SELECT EMPNO, ENAME, TO_CHAR(HIREDATE,'YYYY-MM-DD') DAY FROM EMP;
