...
SELECT ENAME, MAX(SAL) FROM EMP GROUP BY ENAME; -- 무의미
...
SELECT DNAME FROM EMP, DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO AND ENAME = 'SCOTT'; -- JOIN 이용
SELECT DEPTNO FROM EMP WHERE ENAME = 'SCOTT'; -- 서브쿼리
SELECT DNAME FROM DEPT WHERE DEPTNO = (SELECT DEPTNO FROM EMP WHERE ENAME = 'SCOTT'); -- 메인쿼리

    -- 서브쿼리 종류(2) 다중행 서브쿼리(서브쿼리 실행결과가 2행이상) : IN, NOT IN, > ALL, > ANY(SOME), EXISTS
...
SELECT ENAME, SAL FROM EMP E, DEPT D 
    ...
    WHERE E.DEPTNO = D.DEPTNO AND ENAME = 'SCOTT') AND ENAME != 'SCOTT'; -- 메인쿼리
...
SELECT HIREDATE, ENAME FROM EMP
    ...
    -- ex. 최근입사일과 최근 입사자를 출력
SELECT HIREDATE, ENAME FROM EMP
    WHERE HIREDATE = (SELECT MAX(HIREDATE) FROM EMP);
    -- ex. 최초입사일, 최초입사자 이름, 최근 입사일, 최근 입사자 이름
SELECT 
...
SELECT SUM(SAL) FROM EMP WHERE DEPTNO = (SELECT DEPTNO FROM EMP WHERE ENAME = 'SCOTT'); -- 메인쿼리
    -- ex. scott과 동일한 job을 가지는 사원의 모든 필드
SELECT * FROM EMP WHERE JOB = (SELECT JOB FROM EMP WHERE ENAME = 'SCOTT');
    -- ex. DALLAS에서 근무하는 사원의 이름과 부서번호
SELECT ENAME, E.DEPTNO FROM EMP E, DEPT D WHERE E.DEPTNO = D.DEPTNO AND LOC = 'DALLAS'; --EQUI JOIN 이용
SELECT ENAME, DEPTNO FROM EMP WHERE DEPTNO = (SELECT DEPTNO FROM DEPT WHERE LOC = 'DALLAS'); --서브쿼리 이용 
    -- ex> 'KING'이 직속상사인 사원의 이름과 급여
SELECT ENAME, SAL FROM EMP WHERE MGR = (SELECT EMPNO FROM EMP WHERE ENAME = 'KING'); -- 서브쿼리 이용
SELECT E1.ENAME, E1.SAL FROM EMP E1, EMP E2 WHERE E2.ENAME = 'KING' AND E1.MGR = E2.EMPNO; -- SELF JOIN 이용
...
SELECT ENAME, SAL FROM EMP WHERE SAL <= (SELECT AVG(SAL) FROM EMP); -- 메인쿼리
    -- ex2. 평균급여 이하로 받는 사원의 이름과 급여, 평균급여 출력
...
SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP)) - SAL "차이"
    FROM EMP WHERE SAL <= (SELECT AVG(SAL) FROM EMP);
...
-- ★ 3. 다중행 서브쿼리 : IN, ALL, ANY = SOME, EXIST
...
SELECT * FROM EMP WHERE SAL > ALL(SELECT SAL FROM EMP WHERE DEPTNO = 30); -- 다중행 서브쿼리 이용
SELECT * FROM EMP WHERE SAL > (SELECT MAX(SAL) FROM EMP WHERE DEPTNO = 30); -- 단일행 서브쿼리 이용
...
SELECT EMPNO, ENAME, SAL FROM EMP MANAGER
    WHERE EXISTS (SELECT * FROM EMP WHERE MANAGER.EMPNO=MGR); -- 부하가 있는지 존재하면 참
...
-- 탄탄ex3. 급여가 3000이상인 사람들 중 급여 등급을 나누어서 해당 등급별 최고 연봉을 받는 사람들의 사번, 이름, 직업, 입사일, 급여, 급여등급을 출력
...
-- 탄탄ex4. 응용심화 : 입사일 분기별로 가장 높은 급여를 받는 사람들의 분기, 사번, 이름, JOB, 상사사번, 입사일, 급여, 상여를 출력하세요
SELECT HIREDATE, CEIL(EXTRACT(MONTH FROM HIREDATE)/3) "QUARTER" FROM EMP; -- 분기 추출 방법1
SELECT HIREDATE, CEIL(TO_CHAR(HIREDATE,'MM')/3) "QUARTER" FROM EMP; -- 분기 추출 방법2
SELECT HIREDATE, TO_CHAR(HIREDATE,'Q') "QUARTER" FROM EMP; -- 분기 추출 방법 3
...
SELECT JOB, ENAME, EMPNO, DEPTNO 
    FROM EMP
    WHERE ENAME IN (SELECT E2.ENAME FROM EMP E1, EMP E2 WHERE E1.MGR = E2.EMPNO);
    
-- 26.  말단 사원의 사번, 이름, 업무, 부서번호
SELECT EMPNO, ENAME, JOB, DEPTNO
    FROM EMP
    WHERE ENAME IN (SELECT E2.ENAME FROM EMP E1, EMP E2 WHERE E1.MGR(+) = E2.EMPNO AND E1.ENAME IS NULL);
