...
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL <> 3000; -- 다르다
    ...
    -- A < AA < AAA < AAAA < AAAAA < AAAAAA < AAAAAAA < AAAAAA..B
...
SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') >= '82/01/01' AND
TO_CHAR(HIREDATE, 'RR/MM/DD') <= '82/12/31';
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD';
SELECT * FROM EMP;
    -- ex3. 연봉이 2400이상인 직원 ENAME, SAL, 연봉(SAL*12)을 출력
SELECT ENAME, SAL, SAL*12 ANNUALSAL -- (3)
    FROM EMP                     -- (1)
    WHERE ANNUALSAL >= 2400;     -- (2) WHERE절에는 필드 별칭 사용 불가 //에러
SELECT ENAME, SAL, SAL*12 ANNUALSAL -- (3)번째
    FROM EMP                        -- (1)번째
    WHERE SAL*12 > 2400;            -- (2)번째
    --ex4. 연봉이 10,000 이상인 직원의 ENAME, SAL, ANNUALSAL(연봉순으로 정렬)
SELECT ENAME, SAL, SAL*12 연봉 -- (3) 특정 필드만 잘라서
    FROM EMP                -- (1) 이 테이블에서
    WHERE SAL*12 > 2400     -- (2) 조건을 만족하는지 확인후 
    ORDER BY 연봉;           -- (4) 마지막에 정렬
...
-- 5. 산술연산자
SELECT EMPNO, ENAME, SAL, SAL*1.1 UPGRADESEL FROM EMP;
    -- ex. 모든 사원의 이름(ename), 월급(sal), 상여(comm), 연봉(sal*12+comm)을 출력
SELECT ENAME, SAL, COMM, SAL*12+COMM FROM EMP;
    -- 산술연산의 결과는 NULL을 포함하면 결과는 NULL
    -- NVL(NULL일 수도 있는 필드명, 대치값)을 이용  //필드명과 대치값은 타입이 일치해야한다
...
SELECT EMPNO, ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE > '81/02/01' ORDER BY HIREDATE;

    -- ※ 날짜설정이 RR/MM/DD가 아닌경우
...
SELECT EMPNO, ENAME, JOB, DEPTNO FROM EMP WHERE DEPTNO = 10 OR DEPTNO = 30;
    ...
    -- 11. 업   무가 SALESMAN이거나 급여가 3000이상인 사람의 사번, 이름, 업무, 부서코드 출력
...
SELECT  
ENAME || '은 ' || JOB || ' 업무이고 연봉은 ' ||(SAL*12 + NVL(COMM,0)) || '이다' 별칭 FROM EMP;

    -- CF. "ENAME의 상여는 800" (연결연산자로 하면 NULL은 출력 안됨)
SELECT ENAME || '의 상여는 ' || COMM FROM EMP;


-- 8. SQL연산자(BETWEEN, IN, LIKE, IS NULL)
    -- (1) BETWEEN : BETWEEN A AND B ; A부터 B까지 (A,B포함)
        -- ex. SAL이 1500이상 3000이하인 직원의 사번, 이름, 급여 출력
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL >= 1500 AND SAL <= 3000;
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL BETWEEN 1500 AND 3000;
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL BETWEEN 3000 AND 1500; -- 불가능 (작은것부터
        -- ex. SAL이 1500미만, 3000 초과
SELECT * FROM EMP WHERE SAL NOT BETWEEN 1500 AND 3000;
        -- ex. 이름이 'A', 'B', 'C'로 시작하는 직원의 모든 필드
SELECT * FROM EMP WHERE ENAME BETWEEN 'A' AND 'D' AND ENAME != 'D'; -- 'A'부터 'D'까지인데 D는 제외
        -- ex. 82년도에 입사한 직원의 모든 필드 출력 
SELECT * FROM EMP WHERE HIREDATE BETWEEN '82/01/01' AND '82/12/31';

    -- (2) IN
        -- ex. 부서번호가 10, 20, 40 부서인 직원의 모든 필드
...
SELECT * FROM EMP WHERE ENAME LIKE 'M%'; -- M, MA, MAA, MAAZ ....
...
SELECT * FROM EMP WHERE HIREDATE LIKE '82/%';   -- '82/__/__' 와 같음
...
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL DESC, HIREDATE DESC; -- SAL기준으로 내림차순 정렬후 HIREDATE 내림차순 정렬
...
SELECT TO_CHAR(HIREDATE, 'YYYY-MON-DD') FROM EMP;
