...
INSERT INTO EMPv0 VALUES (1111,'홍','ITMANAGER',40); -- VIEW에 INSERT
SELECT * FROM EMPv0;
SELECT * FROM EMP WHERE EMPNO = 1111;
UPDATE EMPv0 SET JOB = 'ANALYST' WHERE EMPNO=1111; -- VIEW UPDATE
SELECT * FROM EMP WHERE EMPNO = 1111;
DELETE FROM EMPv0 WHERE EMPNO = 1111; -- VIEW DELETE
SELECT * FROM EMP WHERE EMPNO = 1111;

    -- EMPv0 이라는 VIEW = EMP의 30번 부서만
CREATE OR REPLACE VIEW EMPv0
    AS SELECT * FROM EMP WHERE DEPTNO = 30;
SELECT * FROM USER_VIEWS;
INSERT INTO EMPv0 VALUES (1111,'홍',NULL,NULL,SYSDATE,NULL,NULL,40); -- 가능
SELECT * FROM EMPv0; -- 30번 부서만 가져오는 뷰인데 40번 부서를 넣어도 보이진 않는다
SELECT * FROM EMP; -- 여기서 확인 가능
INSERT INTO EMPv0 VALUES (1112,'홍',NULL,NULL,SYSDATE,NULL,NULL,30); -- SELECT 보임
SELECT * FROM EMPv0;
SELECT * FROM EMP WHERE EMPNO < 1113;
DELETE FROM EMPv0 WHERE EMPNO < 1113; -- 30번 부서의 WHERE조건이 만족하는 행만 DELETE

    -- 단순뷰에서 INSERT 불가한 경우 : 뷰 생성시 NOT NULL 필드를 미포함한 경우
CREATE OR REPLACE VIEW EMPv0 AS SELECT ENAME, JOB FROM EMP;
INSERT INTO EMPv0 VALUES ('홍','MANAGER'); -- 불가능

-- VIEW의 제한조건
-- WITH CHECK OPTION 추가 : 뷰의 조건에 해당되는 데이터만 삽입, 수정, 삭제가 가능
-- WITH READ ONLY 추가 : 읽기 전용 뷰

    -- ex. emp테이블의 30번 부서의 가상 테이블(30번만 DML 가능)
CREATE OR REPLACE VIEW EMPv0
    AS SELECT * FROM EMP WHERE DEPTNO = 30
    WITH CHECK OPTION;
...
CREATE OR REPLACE VIEW EMPv0
    ...
    WITH READ ONLY;
INSERT INTO EMPv0 (EMPNO,ENAME,DEPTNO) VALUES(1113,'홍',20);


-- (2) 복합뷰 : 두개 이상의 테이블로 구성한 뷰, 가상의 필드를 이용한 경우. DML문을 제한적으로만 사용
-- ① 2개 이상의 테이블을 이용한 복합뷰
CREATE OR REPLACE VIEW EMPv0
...
CREATE OR REPLACE VIEW EMPv0 (EMPNO, ENAME, YEAR_SAL) -- 별칭만 따로 위에 모아서 쓰기
...
SELECT ENAME, RANK() OVER(ORDER BY SAL DESC) RANK,
    ...
    FROM EMP;
-- ROWNUM(테이블로부터 가져온 순서)와 INLINE-VIEW를 이용한 TOP-N
SELECT ROWNUM, ENAME FROM EMP WHERE DEPTNO = 20;
SELECT ROWNUM, ENAME, SAL FROM EMP;
SELECT ROWNUM, ENAME, SAL -- 2
    FROM EMP  -- 1
    ORDER BY SAL; -- 3 -- ROWNUM이 부여된 후에 정렬을 하기때문에 SAL순위가 될수없다  
SELECT ROWNUM, ENAME, SAL
    FROM (SELECT * FROM EMP ORDER BY SAL); -- SAL순서로 정렬을 미리 하고 FROM에서 가져오기때문에 ROWNUM이 순위가 된다
...
SELECT ROWNUM, A.*
    ...
    WHERE ROWNUM <6;
    -- SAL을 기준으로 6등 ~ 10등
SELECT ROWNUM, A.*              -- 3. 즉 행을 하나도 가져오지 못한다
    FROM (SELECT * FROM EMP ORDER BY SAL) A -- 1. 행을 하나씩 가져와서
    WHERE ROWNUM BETWEEN 6 AND 10; -- 2. WHERE절에 조건에 만족하지 않으면 행을 버린다
...
SELECT ROWNUM, B.*
    FROM (SELECT ROWNUM RN, A.* -- ROWNUM에 별칭을 붙히지 않으면 우선순위가 SELECT절이 먼저기때문에 꼬인다
...
CREATE OR REPLACE VIEW DNAME_ENAME_VU
    AS SELECT DNAME, ENAME FROM EMP, DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO;
...
SELECT A.*,ROWNUM "등수"
    FROM (SELECT DEPTNO "부서번호", SUM(SAL) "급여합계"
            FROM EMP 
            GROUP BY DEPTNO 
