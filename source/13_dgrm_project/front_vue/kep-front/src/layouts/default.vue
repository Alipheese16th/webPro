  ...
  <v-app id="app" class="mainHome">
    <v-app-bar id="nav" :clipped-left="$vuetify.breakpoint.lgAndUp" app flat>
      ...
      <div class="nav_wrap--right">
        ...
        <v-btn class="btn_top" @click="siteMap = !siteMap"><img src="~assets/images/btn_top_allmenu.svg" alt="" /></v-btn>
        ...
        <v-list-group v-for="item in subMenus" :key="item.menu_no" no-action
          ...
          <v-list-item v-for="subItem in getSubMenus(item.menu_no)" :key="subItem.menu_no" link class="leftmenu_style"
            ...
            <v-list-item-content class="leftmenu_style_btn">
              <v-list-item-title>{{ subItem.menu_nm }}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list-group>
      </v-list>
...
export default {
  ...
  created() {
    ...
    this.$axios.$get("/api/v1/login/session")
      .then(result => {
        this.$axios.$get("/api/v1/common/whoami", { params: { id: result } })
          .then(res => {
    ...
    this.$axios.$get(`/api/v1/common/menus`).then((res) => {
      ...
      console.log('menu 부서관리: ', res.find(i => i.menu_nm.includes('부서')))
  ...
  methods: {
    ...
    goMenu(url, menuName) {
      ...
      this.$store.dispatch('menu/setCurrent', current)
