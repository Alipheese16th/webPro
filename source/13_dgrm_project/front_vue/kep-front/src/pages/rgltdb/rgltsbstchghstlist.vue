<template>
  <div>
    <div class="location">
      <!-- Home > 규제 DB > 규제 물질 변경 이력 -->
      <span>Home > {{ $t('LB00000218') }} > {{ $t('ME00000008') }}</span>
    </div>
    <!-- 규제 물질 변경 이력 -->
    ...
    <popup ref="pop" mkey1="rgltsbstchghstlistuserslct1" mkey2="rgltsbstchghstlistuserslct2" @close="popclose" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async select() {
      ...
      const rows = await this.$axios.$get(`/api/v1/ch/rgltdb/rglt-sbst-hst-list`, { params: this.schData })
