<template>
  <div>
    ...
    <div class="container_detail">
      ...
      <h3>{{ $t('첨부파일 : 위험성평가 참여인원 (서명지 증빙)') }}</h3>
    ...
    <div class="container_detail">
      <div class="grid_header">
        ...
        <div>
          ...
          <!--다음단계-->
          <v-btn depressed width="120" color="success" height="40" :disabled="frmData.dgsn_apal_drup_stp_cd !== '5'" @click="next">{{ $t('다음단계') }}</v-btn>
          <!--목록-->
  ...
  mounted() {
    ...
    this.$axios
      ...
      .then((res) => {
        ...
        if (this.$route.params.riskevalriskrdtplanlup) {
          this.frmData.dgsn_apal_id = this.$route.params.riskevalriskrdtplanlup
          this.select()
        }
      })
      .catch((err) => {
  ...
  methods: {
    ...
    next() {
      this.$router.push({
        ...
        params: {
          schData: this.schData,
          riskevalcmpllup: this.frmData.dgsn_apal_id,
        },
      })
    },
    goStep(step) {
