<template>
  <div>
    <div class="location">
      <!-- Home > 건강검진 > 특수검진 대상자 관리 -->
      <span>Home > {{ $t('LB00001033') }} > {{ $t('LB00001053') }}</span>
    </div>
    ...
    <popup ref="pop" mkey="hmexspctrgp" @close="popClose" />
  </div>
</template>
...
export default {
  ...
  computed: {
    routeTab() {
      return this.$t('LB00001053') // 특수검진 대상자 관리
    },
    // $t() {
    //   return (cd) => {
  ...
  methods: {
    ...
    async save() {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        rslt.hmex_cl_cd = dataProvider.getValue(items[i], 'hmex_cl_cd')
        rslt.deal_mtra_cd = dataProvider.getValue(items[i], 'deal_mtra_cd')
    ...
    excelExport() {
      ...
      const footer = 'hidden'
      // 특수검진 대상자 목록
      const fileName = this.$t('LB00001056') + '_' + new Date().toISOString().substr(0, 10) + '.xlsx'
