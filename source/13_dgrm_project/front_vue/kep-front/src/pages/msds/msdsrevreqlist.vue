<template>
  <div>
    <div class="location">
      <!-- MSDS 개정요청 -->
      <span>Home > MSDS >{{ $t('LB00000273') }}</span>
    </div>
    <!-- MSDS 개정요청 -->
    ...
    <div class="realgrid_container">
      ...
      <div class="grid_header">
        ...
        <div>
          <v-btn outlined height="28" @click="add">{{ $t('LB00000111') }}</v-btn>
        </div>
      </div>
...
export default {
  ...
  created() {
    ...
    this.frmData.user.dept_cd = user.dept_cd
  },
  mounted() {
    const columns = [
      {
  ...
  methods: {
    ...
    async select() {
      const rows = await this.$axios.$get(`/api/v1/ch/msds/msds-rev`, { params: this.schData })
    ...
    async openDetail(index) {
      ...
      // 현재 위치에서 업데이트 가능 혹은 조회 화면으로 분기해야함.(개정 요청 진행 상태 체크 혹은 dept 동일 여부 체크 필요)
      const auth = await this.$axios.$get(`/api/v1/ch/msds/authchk`, { params: { msds_rev_no: value, dept_cd: this.frmData.user.dept_cd } })
      if (auth !== 'READ') {
        this.$router.push({
