            ...
            <v-row>
              ...
              <v-col cols="3" style="display: flex">
                ...
                <v-menu
                  ...
                  :nudge-right="40"
                  :return-value.sync="frmData.circ_chk_hm"
                ...
                >
                  ...
                  <v-time-picker
                    ...
                    full-width
                    @click:minute="$refs.time1.save(frmData.circ_chk_hm)"
                  ></v-time-picker>
                </v-menu>
              </v-col>
            </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      codeid: ['ST00000002', 'ST00000018', 'SH00000021', 'SH00000022', 'SH00000063', 'SH00000068'],
      codes: {
        ST00000002: [{ cd: '', cd_nm: '' }],
        ST00000018: [{ cd: '', cd_nm: '' }],
        SH00000021: [{ cd: '', cd_nm: '' }],
  ...
  mounted() {
    this.$axios
      ...
      .then((res) => {
        ...
        gridView2.onEditRowChanged = function (grid, itemIndex, dataRow, field, oldValue, newValue) {
          ...
          if (newValue === 'N') {
            ...
            for (let i = 0; i < grid.getItemCount(); i++) {
              ...
              if (chk === 'N' && grid.getValue(i, 'circ_chk_chec_in_cl_cd') !== 'IN01') {
                return
              }
            }
        ...
        // 권한 체크. 같은 부서 여부 조회
        if (!this.isEmpty(this.$route.params.wknstourchkmgntdtledt)) {
          this.$axios.$get(`/api/v1/she/sftchk/auth-check`, { params: { gubun: 'WCHK', circ_chk_id: this.$route.params.wknstourchkmgntdtledt } }).then((res) => {
            if (res === 'Success') {
              this.schData.circ_chk_id = this.$route.params.wknstourchkmgntdtledt
              this.select()
            } else {
  ...
  methods: {
    ...
    async select() {
      ...
      await this.$axios.get('/api/v1/she/sftchk/wknstourchkmgntchkrslt', { params: this.schData }).then((res) => {
        if (res.data !== 'Fail' && res.data !== 'READ') {
          ...
          if (!this.isEmpty(wknsChkResult.circ_chk_hm)) {
            this.$set(this.frmData, 'circ_chk_hm', wknsChkResult.circ_chk_hm.substring(0, 2) + ':' + wknsChkResult.circ_chk_hm.substring(2, 4))
          }
    ...
    validate() {
      if (this.isEmpty(this.frmData.circ_chk_supe_emp_nm)) {
        ...
        return false
      } else if (this.isEmpty(this.frmData.circ_chk_hm)) {
        this.$toast.show(this.$t('MS00000271'), { className: 'toast_error' }) // 점검 시간을 선택해주세요.
        this.$refs.circ_chk_hm.focus()
        return false
      }
      gridView.commit()
