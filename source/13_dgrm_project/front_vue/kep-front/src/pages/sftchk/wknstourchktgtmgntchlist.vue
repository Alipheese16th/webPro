<template>
  <div>
      ...
      <v-row>
        ...
        <v-col cols="2">
          <div class="sch_btn">
            ...
            <!-- 검색 -->
            <v-btn color="primary" depressed height="36" @click="selectCircTgtList">{{ $t('LB00000030') }}</v-btn>
          </div>
        </v-col>
      </v-row>
    </div>
    ...
    <div class="realgrid_container">
      ...
      <popup ref="pop" mkey1="wknstourchktgtmgntchlist1" mkey2="wknstourchktgtmgntchlist2" @close="popClose" />
      <rgnslct ref="pop2" mkey="wknstourchktgtmgntchlist1" :iswkpl="false" :wkplid="schData.wkpl_id" @close="popLocClose" />
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$axios
      ...
      .then((res) => {
        ...
        gridView.setColumnProperty('circ_chk_tgt_id', 'visible', false)
  ...
  methods: {
    ...
    async selectCircTgtList() {
      if (this.isEmpty(this.schData.circ_chk_supe_emp_nm)) {
        this.schData.circ_chk_supe_emp_id = ''
      }
      this.schData.sch_gb = 'tourch' // 유해 화학물질 구분
      const rows = await this.$axios.$get(`/api/v1/she/sftchk/wknstourchktgtmgntlist`, { params: this.schData })
    ...
    getReport() {
      ...
      if (checkedRows.length > 0) {
        for (let i = 0; i < checkedRows.length; i++) {
          keyVal = keyVal + "'" + dataProvider.getValue(checkedRows[i], 'circ_chk_tgt_id') + "',"
        }
    ...
    openDetail(index) {
      ...
      if (index.column === 'circ_chk_tgt_nm') {
        const value = gridView.getValue(index.itemIndex, 'circ_chk_tgt_id')

        // 권한 체크
        if (!this.isEmpty(value)) {
          this.$axios.$get(`/api/v1/she/sftchk/auth-check`, { params: { gubun: 'WKTGT', circ_chk_tgt_id: value } }).then((res) => {
            // eslint-disable-next-line no-console
            if (res === 'Success') {
              this.$router.push({
                name: `sftchk-wknstourchktgtmgntchdtledt-wknstourchktgtmgntchdtledt`,
                params: {
                  wknstourchktgtmgntdtledt: value,
                  tmpSchData: {
                    wkpl_id: this.schData.wkpl_id,
                    circ_chk_tgt_nm: this.schData.circ_chk_tgt_nm,
                    circ_chk_supe_emp_nm: this.schData.circ_chk_supe_emp_nm,
              ...
              this.$router.push({
                ...
                params: {
                  wknstourchktgtmgntdtllup: value,
                  tmpSchData: {
                    wkpl_id: this.schData.wkpl_id,
                    circ_chk_tgt_nm: this.schData.circ_chk_tgt_nm,
                    circ_chk_supe_emp_nm: this.schData.circ_chk_supe_emp_nm,
