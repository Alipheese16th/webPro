<template>
  <div>
    <div class="location">
      <!-- 안전 점검 및 개선 --><!-- 작업장 순회점검 관리-->
      <span>Home > {{ $t('LB00000735') }} > {{ $t('LB00000911') }}</span>
    </div>
...
export default {
  ...
  methods: {
    ...
    async selectCircChkList() {
      ...
      this.schData.sch_gb = 'tour' // 유해물질 구분
    ...
    async aprv() {
      ...
      for (let i = 0; i < items.length; i++) {
        const value = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_prst_cd')
        if (value !== '40') {
        ...
        // 결제 정보 선언
        const aprvInfo = {}
        aprvInfo.circ_chk_id = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_id')
        aprvInfo.wkpl_nm = dataProvider.getValue(gridView.getDataRow(items[i]), 'wkpl_nm')
        aprvInfo.circ_chk_tgt_nm = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_tgt_nm')
        aprvInfo.circ_chk_wkpl_loc_nm = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_wkpl_loc_nm')
        aprvInfo.circ_chk_dtl_loc_nm = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_dtl_loc_nm')
        aprvInfo.circ_chk_dttm = dataProvider.getValue(gridView.getDataRow(items[i]), 'circ_chk_dttm')
        aprvInfo.circ_chk_supe_emp_nm = gridView.getValue(gridView.getDataRow(items[i]), 'circ_chk_supe_emp_nm')
    ...
    excelExport() {
      ...
      const footer = 'hidden'
      const fileName = this.$t('LB00000914') + new Date().toISOString().substr(0, 10) + '.xlsx'
