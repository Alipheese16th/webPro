<template>
  <div>
      ...
      <v-row>
        <v-col cols="10">
          <v-container>
            ...
            <v-row>
              ...
              <v-col cols="3" style="display: flex">
                <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="schData.jnt_chk_schd_st_dt"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                  ...
                  <v-date-picker v-model="schData.jnt_chk_schd_st_dt" no-title @input="menu1 = false"> </v-date-picker>
                </v-menu>
                <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="schData.jnt_chk_schd_ed_dt"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                  ...
                  <v-date-picker v-model="schData.jnt_chk_schd_ed_dt" no-title @input="menu2 = false"> </v-date-picker>
                </v-menu>
              </v-col>
              <v-col cols="1">
                <!--점검코스-->
              ...
              <v-col cols="3">
                <v-text-field
                  ref="jnt_chk_wkpl_loc_nm"
                  v-model="schData.jnt_chk_wkpl_loc_nm"
                  outlined
                  single-line
                  :hide-details="true"
                  dense
              ...
              <v-col cols="3">
                <v-select
                  ref="jnt_chk_prst_cd"
                  v-model="schData.jnt_chk_prst_cd"
                  label="ALL"
                  item-text="cd_nm"
                  item-value="cd"
        ...
        <v-col cols="2">
          <div class="sch_btn">
            ...
            <v-btn color="primary" depressed height="36" @click="selectJntChkList">{{ $t('LB00000030') }}</v-btn>
          </div>
        </v-col>
      </v-row>
    </div>
    ...
    <popup ref="pop" mkey1="wknscoprchkmgntlist1" mkey2="wknscoprchkmgntlist2" @close="popClose" />
    <rgnslct ref="pop2" mkey="wknscoprchkmgntlist1" @close="popLocClose" />>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.schData.jnt_chk_schd_st_dt = this.get_date_str_gubun(firstDay, '-')
    this.schData.jnt_chk_schd_ed_dt = this.get_date_str_gubun(lastDay, '-')
  ...
  methods: {
    ...
    async selectJntChkList() {
      if (!this.isEmpty(this.schData.jnt_chk_schd_st_dt)) {
        this.schData.sch_jnt_chk_schd_st_dt = this.toDateStringfromGubun(this.schData.jnt_chk_schd_st_dt, '-')
      }
      if (!this.isEmpty(this.schData.jnt_chk_schd_ed_dt)) {
        this.schData.sch_jnt_chk_schd_ed_dt = this.toDateStringfromGubun(this.schData.jnt_chk_schd_ed_dt, '-')
      }
      if (this.isEmpty(this.schData.jnt_chk_supe_emp_nm)) {
        this.schData.jnt_chk_supe_emp_id = ''
      }
      if (this.schData.sch_jnt_chk_schd_st_dt > this.schData.sch_jnt_chk_schd_ed_dt) {
        this.$toast.show(this.$t('MS00000395'), { className: 'toast_error' }) // 시작일이 종료일보다 클 수 없습니다.
        return false
      }
      const rows = await this.$axios.$get(`/api/v1/she/sftchk/wknscoprchkmgntlist`, { params: this.schData })
    ...
    openDetail(index) {
      ...
      if (index.column === 'jnt_chk_tgt_nm') {
        const value = gridView.getValue(index.itemIndex, 'jnt_chk_id')

        // 권한 체크
        if (!this.isEmpty(value)) {
          this.$axios.$get(`/api/v1/she/sftchk/auth-check`, { params: { gubun: 'JCHK', jnt_chk_id: value } }).then((res) => {
            // eslint-disable-next-line no-console
            if (res === 'Success') {
              this.$router.push({
                name: `sftchk-wknscoprchkmgntdtledt-wknscoprchkmgntdtledt`,
                params: {
                  wknscoprchkmgntdtledt: value,
                  tmpSchData: {
                    wkpl_id: this.schData.wkpl_id,
                    jnt_chk_tgt_nm: this.schData.jnt_chk_tgt_nm,
                    jnt_chk_schd_st_dt: this.schData.jnt_chk_schd_st_dt,
                    jnt_chk_schd_ed_dt: this.schData.jnt_chk_schd_ed_dt,
                    jnt_chk_supe_emp_nm: this.schData.jnt_chk_supe_emp_nm,
                    jnt_chk_supe_emp_id: this.schData.jnt_chk_supe_emp_id,
                    jnt_chk_wkpl_loc_id: this.schData.jnt_chk_wkpl_loc_id,
                    jnt_chk_wkpl_loc_nm: this.schData.jnt_chk_wkpl_loc_nm,
                    jnt_chk_prst_cd: this.schData.jnt_chk_prst_cd,
    ...
    excelExport() {
      ...
      const footer = 'hidden'
      const fileName = this.$t('LB00001059') + new Date().toISOString().substr(0, 10) + '.xlsx'
