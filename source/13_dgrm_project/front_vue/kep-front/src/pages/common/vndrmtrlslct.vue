  ...
  <v-dialog v-model="dialog" persistent width="1000px" :eager="true">
    <v-card class="container_pop">
      <v-toolbar flat class="mb-6">
        <!-- 구매처 선택 -->
        <v-toolbar-title>{{ $t('LB00000297') }}</v-toolbar-title>
        <v-spacer></v-spacer>
        <!-- 닫기 -->
      ...
      <div class="container_grid">
        <div class="realgrid_container">
          ...
          <div id="realgrid_pop03" style="width: 100%; height: 281px"></div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    const columns = [
      {
        header: this.$t('LB00000298'), // 구매처코드
        name: 'vndr_no',
        fieldName: 'vndr_no',
        width: '100',
      },
      ...
      {
        header: this.$t('LB00000300'), // 최근구매일
        name: 'vndr_dt',
        fieldName: 'vndr_dt',
        width: '100',
      },
    ...
    const gridId = document.getElementById('realgrid_pop03')
    gridId.id = this.mkey + 'vndrmtrlslct'
    this.gridView = new GridView(this.mkey + 'vndrmtrlslct')
