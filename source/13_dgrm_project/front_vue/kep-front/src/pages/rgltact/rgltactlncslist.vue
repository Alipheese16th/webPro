<template>
  <div>
    <div class="location">
      <!--규제대응--><!--인허가-->
      <span>Home > {{ $t('MN00000016') }} > {{ $t('LB00000618') }}</span>
    </div>
...
export default {
  ...
  mounted() {
    ...
    const columns = [
      {
        header: this.$t('LB00000499'), // 인허가 구분
        name: 'doc_type_cd_nm',
    ...
    gridView.onCellClicked = function (grid, clickData) {
      const value = dataProvider.getValue(clickData.dataRow, 'bbs_doc_no')
      this.searchdatas.bbs_doc_no = value
      // 상세 or 수정 화면으로 이동
      if (clickData.cellType === 'data' && clickData.column === 'bbs_doc_ttl') {
        if (this.searchdatas.userApi) {
          this.$router.push({
            name: 'rgltact-rgltactlncsedt-rgltactlncsedt',
            params: {
              rgltactlncsedt: this.searchdatas.bbs_doc_no,
              schData: {
          ...
          this.$router.push({
            ...
            params: {
              rgltactlncslup: this.searchdatas.bbs_doc_no,
              schData: {
                bbs_doc_ttl: this.searchdatas.bbs_doc_ttl,
                bbs_doc_cntn: this.searchdatas.bbs_doc_cntn,
                doc_type_cd: this.searchdatas.doc_type_cd,
      ...
      if (clickData.column === 'atfile_no') {
        const key = dataProvider.getValue(clickData.dataRow, 'atfile_no')
  ...
  methods: {
    ...
    async select() {
      ...
      const rows = await this.$axios.$get(`/api/v1/ch/rgltact/lncs`, { params: this.searchdatas })
