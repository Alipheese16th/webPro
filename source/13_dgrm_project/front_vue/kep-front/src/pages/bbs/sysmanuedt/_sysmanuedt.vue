...
export default {
  ...
  computed: {
    routeTab() {
      return this.$t('LB00000129') + ' ' + this.$t('LB00000111')
    },
    // $t() {
    //   return (cd) => {
  ...
  async created() {
    ...
    if (this.$route.params.sysmanuedt !== null && this.$route.params.sysmanuedt !== undefined) {
      this.searchdatas.bbs_doc_no = await this.$route.params.sysmanuedt
      this.selectDtl()
    } else {
      this.onEditorChange()
    }
  ...
  methods: {
    ...
    async selectDtl() {
      const data = await this.$axios.$get(`/api/v1/common/bbs/manu/${this.searchdatas.bbs_doc_no}`)
      ...
      if (files.length > 0) {
        ...
        this.tempData.fileCount = files.length
      } else {
        this.tempData.fileCount = 0
      }
    ...
    async save() {
      if (await this.validationChk()) {
        ...
        if (await !savenext) {
          ...
          this.$axios.post('/api/v1/common/bbs/manu', this.savedatas).then((res) => {
    ...
    async del() {
      ...
      await this.$axios.delete(`/api/v1/common/bbs/manu/${this.searchdatas.bbs_doc_no}`).then((res) => {
        if (res.data === 'Success') {
