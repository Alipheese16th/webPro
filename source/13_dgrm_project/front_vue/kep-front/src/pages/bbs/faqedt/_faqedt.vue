<template>
  <div>
    <div class="location">
      <!--커뮤니티(KO)--><!--FAQ(KO)-->
      <span>Home > {{ $t('LB00000117') }} > {{ $t('LB00000124') }}</span>
    </div>
    <!--FAQ(KO)-->
...
export default {
  meta: {
    title: 'FAQ 등록',
    key(route) {
      return `/bbs/faqs/${route.params.catalog}`
    },
  },
  components: {
    quillEditor,
  ...
  computed: {
    routeTab() {
      return this.$t('LB00000124') + ' ' + this.$t('LB00000111')
    },
    // $t() {
    //   return (cd) => {
  ...
  async created() {
    ...
    if (this.$route.params.faqedt !== null && this.$route.params.faqedt !== undefined) {
      this.searchdatas.bbs_doc_no = await this.$route.params.faqedt
      this.selectDtl()
    } else {
      this.onEditorChange()
    }
  ...
  methods: {
    ...
    async selectDtl() {
      const data = await this.$axios.$get(`/api/v1/common/bbs/faqs/${this.searchdatas.bbs_doc_no}`)
    ...
    async save() {
      if (await this.validationChk()) {
        ...
        if (await !savenext) {
          ...
          this.$axios.post('/api/v1/common/bbs/faqs', this.savedatas).then((res) => {
    ...
    async del() {
      ...
      await this.$axios.delete(`/api/v1/common/bbs/faqs/${this.searchdatas.bbs_doc_no}`).then((res) => {
        if (res.data === 'Success') {
