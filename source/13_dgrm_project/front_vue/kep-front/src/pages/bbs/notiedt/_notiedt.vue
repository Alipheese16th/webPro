<template>
  <div>
    <div class="location">
      <!--커뮤니티(KO)--><!--공지사항(KO)-->
      <span>Home > {{ $t('LB00000117') }} > {{ $t('LB00000116') }}</span>
    </div>
    <div class="work_title">
      <!--공지사항(KO)-->
      <h2>{{ $t('LB00000116') }}</h2>
    </div>
    <div class="container_detail">
      <div class="sub_title">
        <!--공지사항(KO)-->
        <h3>{{ $t('LB00000116') }}</h3>
      </div>
      <v-row>
        <v-col cols="12">
          <v-container>
            <v-row>
              <v-col cols="1">
                <!--제목(KO)-->
                <div class="label_tit">{{ $t('LB00000114') }}</div>
              </v-col>
              <v-col cols="3">
                <!--입력하세요-->
                <v-text-field
                  v-model="savedatas.bbs_doc_ttl"
                  outlined
                  single-line
                  :hide-details="true"
                  dense
                  :placeholder="$t('LB00000033')"
                  height="30"
                  maxlength="199"
                ></v-text-field>
              </v-col>
              <v-col cols="1">
                <!--등록자-->
                <div class="label_tit">{{ $t('LB00000055') }}</div>
              </v-col>
              <v-col cols="3">
                <div class="label_con">{{ savedatas.username }}</div>
              </v-col>
              <v-col cols="1">
                <!--등록일-->
                <div class="label_tit">{{ $t('LB00000054') }}</div>
              </v-col>
              <v-col cols="3">
                <div class="label_con">{{ savedatas.date }}</div>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="1">
                <!--업무구분(KO)-->
                <div class="label_tit">{{ $t('LB00000052') }}</div>
              </v-col>
              <v-col cols="3">
                <v-select
                  v-model="savedatas.biz_cd"
                  label="ALL"
                  item-text="cd_nm"
                  item-value="cd"
                  :items="codes.CO00000001"
                  outlined
                  single-line
                  :hide-details="true"
                  dense
                  height="30"
                ></v-select>
              </v-col>
              <v-col cols="1">
                <!--팝업공지-->
                <div class="label_tit">{{ $t('팝업공지') }}</div>
              </v-col>
              <v-col cols="3" class="dp_flex">
                <v-checkbox v-model="savedatas.popup_chk" :hide-details="true" height="30"></v-checkbox>
                <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="savedatas.sDate_pop"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                      :disabled="!savedatas.popup_chk"
                      dense
                      height="30"
                      v-bind="attrs"
                      class="comp_calendar mr-1"
                      v-on="on"
                    ></v-text-field>
                  </template>
                  <v-date-picker v-model="savedatas.sDate_pop" :max="savedatas.eDate_pop" no-title @input="menu1 = false"> </v-date-picker>
                </v-menu>
                <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="savedatas.eDate_pop"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                      :disabled="!savedatas.popup_chk"
                      dense
                      height="30"
                      v-bind="attrs"
                      class="comp_calendar"
                      v-on="on"
                    ></v-text-field>
                  </template>
                  <v-date-picker v-model="savedatas.eDate_pop" :min="savedatas.sDate_pop" no-title @input="menu2 = false"> </v-date-picker>
                </v-menu>
              </v-col>
              <v-col cols="1">
                <!--상단노출-->
                <div class="label_tit">{{ $t('상단노출') }}</div>
              </v-col>
              <v-col cols="3" class="dp_flex">
                <v-checkbox v-model="savedatas.top_chk" :hide-details="true" height="30"></v-checkbox>
                <v-menu v-model="menu3" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="savedatas.sDate_top"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                      :disabled="!savedatas.top_chk"
                      dense
                      height="30"
                      v-bind="attrs"
                      class="comp_calendar mr-1"
                      v-on="on"
                    ></v-text-field>
                  </template>
                  <v-date-picker v-model="savedatas.sDate_top" :max="savedatas.eDate_top" no-title @input="menu3 = false"> </v-date-picker>
                </v-menu>
                <v-menu v-model="menu4" :close-on-content-click="false" :nudge-right="40" min-width="290px">
                  <template v-slot:activator="{ on, attrs }">
                    <v-text-field
                      v-model="savedatas.eDate_top"
                      prepend-icon="event"
                      readonly
                      outlined
                      :hide-details="true"
                      :disabled="!savedatas.top_chk"
                      dense
                      height="30"
                      v-bind="attrs"
                      class="comp_calendar"
                      v-on="on"
                    ></v-text-field>
                  </template>
                  <v-date-picker v-model="savedatas.eDate_top" :min="savedatas.sDate_top" no-title @input="menu4 = false"> </v-date-picker>
                </v-menu>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12" style="padding: 20px 0 !important">
                <quill-editor v-model="savedatas.bbs_doc_cntn" :options="editorOption" :disabled="quillChk" @change="onEditorChange" />
              </v-col>
            </v-row>
          </v-container>
          <div class="group_padd"></div>
          <!--첨부파일 목록(KO)-->
          <h3>{{ $t('LB00000120') }}</h3>
          <div class="container_filelist">
            <table>
              <div v-if="true">
                <file-upload ref="upload" :multiple="true" @onDrop="chkFile()" />
              </div>
            </table>
          </div>
        </v-col>
      </v-row>
    </div>
    <div class="group_padd">&nbsp;</div>
    <div class="group_padd">&nbsp;</div>
    <div class="container_button">
      <!--저장-->
      <v-btn depressed width="120" color="primary" height="40" @click="save">{{ $t('LB00000045') }}</v-btn>
      <!--삭제-->
      <v-btn v-if="searchdatas.bbs_doc_no != ''" depressed width="120" color="primary" height="40" @click="del">{{ $t('LB00000046') }}</v-btn>
      <!--목록-->
      <v-btn outlined width="120" height="40" @click="goList">{{ $t('LB00000118') }}</v-btn>
    </div>
  </div>
</template>
<script>
import { quillEditor } from 'vue-quill-editor'
import FileUpload from '~/components/FileUpload.vue'

export default {
  meta: {
    title: '공지사항 등록',
    key(route) {
      return `/bbs/noti/${route.params.catalog}`
    },
  },
  components: {
    quillEditor,
    FileUpload,
  },
  props: {},
  data() {
    return {
      codeid: ['CO00000001'],
      codes: {
        CO00000001: [{ cd: '', cd_nm: '' }],
      },
      selCO00000001: {
        cd: [],
        cd_nm: [],
      },
      usrinfo: {},
      quillChk: true,
      searchdatas: {
        bbs_doc_no: '',
      }, // 목록 화면에서 보내준 데이터
      savedatas: {
        bbs_type_cd: 'NOTI',
        doc_type_cd: '',
        bbs_doc_ttl: '',
        username: '', // 보여주기만 하는 데이터 수정 불가 java에서 session id insert ,update
        upt_emp_no: '', // 보여주기만 하는 데이터 수정 불가 java에서 session id insert ,update
        date: '', // 보여주기만 하는 데이터 수정 불가 java에서 현재 날짜 session id insert ,update
        noti_yn: 'N',
        biz_cd: '',
        bbs_doc_cntn: '',
        atfile_no: '',
        bbs_doc_no: '',
      },
      menu1: false,
      menu2: false,
      menu3: false,
      menu4: false,
      editorOption: {
        // some quill options
        modules: {
          toolbar: [
            ['bold', 'italic', 'underline', 'strike'], // toggled buttons
            ['blockquote', 'code-block'],

            [{ list: 'ordered' }, { list: 'bullet' }],
            [{ indent: '-1' }, { indent: '+1' }], // outdent/indent

            [{ header: [1, 2, 3, 4, 5, 6, false] }],

            [{ color: [] }, { background: [] }], // dropdown with defaults from theme
            [{ align: [] }],

            ['link', 'image'],

            ['clean'], // remove formatting button
          ],
        },
        placeholder: '',
      },
      tempData: {
        bbs_doc_ttl: '',
        noti_yn: 'N',
        biz_cd: '',
        bbs_doc_cntn: '',
        fileModify: false, // 첨부파일 수정 여부
      },
    }
  },
  computed: {
    routeTab() {
      return this.$t('LB00000116') + ' ' + this.$t('LB00000111')
    },
    // $t() {
    //   return (cd) => {
    //     if (cd.startsWith('LB') || cd.startsWith('MS')) return this.$store.getters['mlang/getMultiLanguage'](cd)
    //     else return cd
    //     // return this.$store.getters['mlang/getMultiLanguage'](cd)
    //   }
    // },
  },
  async beforePageLeave(tab, type) {
    if (
      this.tempData.bbs_doc_ttl !== this.savedatas.bbs_doc_ttl ||
      this.tempData.noti_yn !== this.savedatas.noti_yn ||
      this.tempData.biz_cd !== this.savedatas.biz_cd ||
      this.tempData.bbs_doc_cntn !== this.savedatas.bbs_doc_cntn ||
      this.tempData.fileModify === true
    ) {
      const res = await this.$confirm(this.$t('MS00000060'))
      return new Promise((resolve, reject) => {
        if (res) {
          resolve()
        } else {
          // eslint-disable-next-line prefer-promise-reject-errors
          reject('reject')
        }
      })
    }
  },
  mounted() {
    this.$axios
      .$get(`/api/v1/comm/comm-cd`, { params: this.codeid })
      .then((res) => {
        this.codes = res
        this.selCO00000001.cd.push('')
        this.selCO00000001.cd_nm.push('SELECT')
        res.CO00000001.forEach((i) => {
          this.selCO00000001.cd.push(i.cd)
          this.selCO00000001.cd_nm.push(i.cd_nm)
        })

        this.codes.CO00000001.unshift({ cd: '', cd_nm: 'ALL' })
      })
      .catch((err) => {
        // eslint-disable-next-line no-console
        console.log(err)
      })
    this.usrinfo = this.$store.getters['auth/getUser']
    this.savedatas.username = this.usrinfo.emp_nm
    this.savedatas.upt_emp_no = this.usrinfo.emp_no
    this.savedatas.date = this.getTzDate(this.usrinfo.time_zone, '-')

    // 파라미터 조회 - 파라미터가 존재할 경우
    if (this.$route.params.notiedt !== null && this.$route.params.notiedt !== undefined) {
      this.searchdatas.bbs_doc_no = this.$route.params.notiedt
      this.selectDtl()
    } else {
      this.savedatas.sDate_pop = this.getTzDate(this.usrinfo.time_zone, '-')
      this.savedatas.eDate_pop = this.getTzDate(this.usrinfo.time_zone, '-', -30)
      this.savedatas.sDate_top = this.getTzDate(this.usrinfo.time_zone, '-')
      this.savedatas.eDate_top = this.getTzDate(this.usrinfo.time_zone, '-', -30)
      this.onEditorChange()
    }
    // 이전 검색 결과 저장
    if (this.$route.params.schData !== null && this.$route.params.schData !== undefined) {
      this.tmpSchData = this.$route.params.schData
    } else {
      this.tmpSchData = {
        bbs_doc_ttl: '',
        bbs_doc_cntn: '',
        noti_yn: '',
        biz_cd: '',
        sDate: this.getTzDate(this.usrinfo.time_zone, '-', 365),
        eDate: this.getTzDate(this.usrinfo.time_zone, '-'),
        emp_nm: '',
      }
    }
  },
  methods: {
    chkFile() {
      this.tempData.fileModify = true
    },
    async selectDtl() {
      const data = await this.$axios.$get(`/api/v1/common/bbs/noti/${this.searchdatas.bbs_doc_no}`)

      this.savedatas.bbs_doc_ttl = data.bbs_doc_ttl
      this.savedatas.bbs_doc_cntn = data.bbs_doc_cntn
      this.savedatas.username = data.emp_nm
      this.savedatas.upt_emp_no = data.upt_emp_no
      this.savedatas.date = data.upt_dt
      this.savedatas.noti_yn = data.noti_yn
      this.savedatas.biz_cd = data.biz_cd
      this.savedatas.atfile_no = data.atfile_no
      this.savedatas.bbs_doc_no = data.bbs_doc_no
      if (data.noti_yn === 'Y') {
        this.$set(this.savedatas, 'popup_chk', true)
        this.$set(this.savedatas, 'sDate_pop', data.ppup_bgn_dt)
        this.$set(this.savedatas, 'eDate_pop', data.ppup_end_dt)
      } else {
        this.$set(this.savedatas, 'popup_chk', false)
        this.$set(this.savedatas, 'sDate_pop', this.getTzDate(this.usrinfo.time_zone, '-'))
        this.$set(this.savedatas, 'eDate_pop', this.getTzDate(this.usrinfo.time_zone, '-', -30))
      }
      if (!data.tppg_fixt_bgn_dt || data.tppg_fixt_bgn_dt === '--') {
        this.$set(this.savedatas, 'top_chk', false)
        this.$set(this.savedatas, 'sDate_top', this.getTzDate(this.usrinfo.time_zone, '-'))
        this.$set(this.savedatas, 'eDate_top', this.getTzDate(this.usrinfo.time_zone, '-', -30))
      } else {
        this.$set(this.savedatas, 'top_chk', true)
        this.$set(this.savedatas, 'sDate_top', data.tppg_fixt_bgn_dt)
        this.$set(this.savedatas, 'eDate_top', data.tppg_fixt_end_dt)
      }

      this.tempData.bbs_doc_ttl = data.bbs_doc_ttl
      this.tempData.noti_yn = data.noti_yn
      this.tempData.biz_cd = data.biz_cd
      this.tempData.bbs_doc_cntn = data.bbs_doc_cntn

      let files = []
      if (!this.isEmpty(this.savedatas.atfile_no)) {
        files = await this.$axios.$get(`/api/v1/common/file/${this.savedatas.atfile_no}`)
        console.log('files : ', files) // eslint-disable-line no-console
      }
      if (files.length > 0) {
        await this.setFiles(files)
      }
      await this.changeEditMode(true)
      this.tempData.fileModify = false // 첨부파일 수정 여부 초기화
    },
    goList() {
      this.$router.push({
        name: `bbs-notilist`,
        params: {
          schData: {
            bbs_doc_ttl: this.tmpSchData.bbs_doc_ttl,
            bbs_doc_cntn: this.tmpSchData.bbs_doc_cntn,
            noti_yn: this.tmpSchData.noti_yn,
            biz_cd: this.tmpSchData.biz_cd,
            sDate: this.tmpSchData.sDate,
            eDate: this.tmpSchData.eDate,
            emp_nm: this.tmpSchData.emp_nm,
          },
        },
      })
    },
    validationChk() {
      if (!this.savedatas.bbs_doc_ttl) {
        this.$toast.show(this.$t('MS00000034'), { className: 'toast_error' }) // 제목을 입력해 주세요.(KO)
        return false
      }
      if (!this.savedatas.biz_cd) {
        this.$toast.show(this.$t('MS00000018'), { className: 'toast_error' }) // 업무구분을 선택해 주세요.(KO)
        return false
      }
      if (!this.savedatas.bbs_doc_cntn) {
        this.$toast.show(this.$t('MS00000032'), { className: 'toast_error' }) // 내용을 입력해 주세요.(KO)
        return false
      }

      return true
    },
    async save() {
      if (!this.validationChk()) {
        return
      }

      const res = await this.$confirm(this.$t('MS00000010')) // 저장하시겠습니까?
      if (!res) {
        return
      }

      if (this.savedatas.popup_chk) {
        this.savedatas.noti_yn = 'Y'
      } else {
        this.savedatas.noti_yn = 'N'
        this.savedatas.sDate_pop = ''
        this.savedatas.eDate_pop = ''
      }

      if (!this.savedatas.top_chk) {
        this.savedatas.sDate_top = ''
        this.savedatas.eDate_top = ''
      }

      this.$axios.post('/api/v1/common/bbs/noti', this.savedatas).then((res) => {
        if (res.data !== 'Fail' && res.data !== 'READ') {
          this.$toast.show(this.$t('MS00000011')) // 저장되었습니다.
          this.searchdatas.bbs_doc_no = res.data
          this.selectDtl()
        } else {
          this.$toast.show(this.$t('MS00000014'), { className: 'toast_error' }) // 오류가 발생했습니다.
        }
      })
    },
    async del() {
      const res = await this.$confirm(this.$t('MS00000037')) // 삭제하시겠습니까?(KO)
      if (!res) {
        return
      }
      this.$axios.delete(`/api/v1/common/bbs/noti/${this.searchdatas.bbs_doc_no}`).then((res) => {
        if (res.data === 'Success') {
          this.$toast.show(this.$t('MS00000041')) // 삭제되었습니다.
          this.goList()
        } else {
          this.$toast.show(this.$t('MS00000014'), { className: 'toast_error' }) // 오류가 발생했습니다.
        }
      })
    },
    isEmpty(str) {
      if (typeof str === 'undefined' || str == null || str === '' || str === 0) return true
      else return false
    },
    changeEditMode(mode) {
      this.$refs.upload.setEdit(mode)
    },
    setFiles(files) {
      this.$nextTick(() => {
        this.$refs.upload.setFiles(files)
        this.$refs.upload.setEdit(false)
      })
    },
    onEditorChange(editor) {
      this.quillChk = false
    },
  },
}
</script>
<style lang="scss" scoped></style>
