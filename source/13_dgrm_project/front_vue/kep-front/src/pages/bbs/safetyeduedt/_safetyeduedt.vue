<template>
  <div>
    <div class="location">
      <!--커뮤니티(KO)--><!--안전교육자료(KO)-->
      <span>Home > {{ $t('LB00000117') }} > {{ $t('MN00001069') }}</span>
    </div>
...
export default {
  ...
  computed: {
    routeTab() {
      return this.$t('MN00001069') + ' ' + this.$t('LB00000111')
    },
    // $t() {
    //   return (cd) => {
  ...
  async created() {
    ...
    if (this.$route.params.archedt !== null && this.$route.params.archedt !== undefined) {
      this.searchdatas.bbs_doc_no = await this.$route.params.archedt
      this.selectDtl()
    } else {
      this.onEditorChange()
    }
  ...
  methods: {
    ...
    async save() {
      if (await this.validationChk()) {
        ...
        if (await !savenext) {
          ...
          this.$axios.post('/api/v1/common/bbs/arch', this.savedatas).then((res) => {
    ...
    async del() {
      ...
      await this.$axios.delete(`/api/v1/common/bbs/arch/${this.searchdatas.bbs_doc_no}`).then((res) => {
        if (res.data === 'Success') {
