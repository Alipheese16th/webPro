...
export default {
  ...
  created() {
    ...
    console.log(this.$route.params.boardedt !== "");

    if (this.$route.params.boardedt !== "") {
      // 수정모드
      this.$axios.get(`/api/v1/public/sbbs/info/${this.$route.params.boardedt}`)
        .then(result => {
  ...
  methods: {
    ...
    create() {
      this.$axios.post("/api/v1/public/sbbs/insertSelectKey", this.boardForm)
        .then(result => {
    ...
    remove() {
      this.$axios.delete(`/api/v1/public/sbbs/delete/${this.boardForm.boardId}`)
        .then(result => {
          if (result.status === 200) {
            alert("삭제되었습니다.");
            this.goList();
