...
DELETE FROM MEMO 
  WHERE MGROUP=2 AND (MSTEP>=1 AND MSTEP<(SELECT MIN(MSTEP) FROM MEMO WHERE MGROUP=2 AND MSTEP>1 AND MINDENT<=1));
...
DELETE FROM MEMO WHERE MGROUP=2 AND (MSTEP>=0 AND 
  MSTEP<(SELECT NVL(MIN(MSTEP),99) FROM MEMO WHERE MGROUP=2 AND MSTEP>0 AND MINDENT<=0));
